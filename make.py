...
img_focal_length = exif['FocalLength']

# Create border
path_new = f"{path.split('.')[0]}_new.jpg"
...
img_model_font = ImageFont.truetype('Arial Black.ttf', 100)
img_model_position = (int(border_width * 0.05), int(border_height * 0.15))
...
img_date_position = (int(border_width * 0.05), int(border_height * 0.55))
...
exif_new = { ExifTags.TAGS[k]: v for k, v in Image.open(f'{path}_new.jpg')._getexif().items() if k in ExifTags.TAGS }
img_new_format = img_new.format
img_new_mode = img_new.mode
img_new_size = os.path.getsize(f'{path}_new.jpg') / (1024 * 1024)
...
img_new_focal_length = exif_new['FocalLength']
